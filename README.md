...
![知识收集库产品截图](docs/images/product-screenshot.png)

## 它解决什么问题

收藏夹的问题不是太满，而是从来不会被重新读到。

知识收集库把“看到有价值内容的瞬间”变成一张本地卡片：原文片段、页面来源、上下文、标签和你的想法会一起落到本机文件夹里。后续可以被 Codex、Claude Code、Kimi Agent、WorkBuddy 等 agent 渐进式读取，而不是把整库一次性塞进上下文。
