...
设计系统：**荧光手账 / Highlighter Journal**（暖纸 `#FFFDF9` + 单一柿子橙强调色 `#E8623C`）。
...
- 句柄持久化在 IndexedDB（`handle-store.js`）。浏览器重启后权限可能回到 `prompt`，需用户在侧边栏点「重新连接文件夹」重新授权；其间保存进 `kcQueue`，连接后回灌，**不丢数据**。
...
| `popup.html/.js/.css` | 侧边栏。`popup.js` 持 `dirHandle`，直接用 `fsstore` 列表/编辑/删除；管理文件夹连接生命周期（query/requestPermission/选择/更换）；连接后回灌 `kcQueue`。`popup.css` = 荧光手账设计系统（**整文件由外部设计稿覆盖,改动需对齐设计**）。 |
...
- 浏览器重启后 FSA 权限可能失效 → 侧边栏「重新连接文件夹」。保存期间走 `kcQueue` 兜底。
...
- ✅ 已完成：FSA 本地优先架构、卡片瘦身 + 一行流索引 + agent 渐进式披露协议、荧光手账 UI、Agent 提示（路径可填一次自动补全）。`npm test` 24/24。
