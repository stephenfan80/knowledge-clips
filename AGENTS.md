...
npm test          # 26 个测试：store.test + clip-format(纯函数+漂移守卫) + fsstore(内存版 FSA mock) + popup 回归
```
...
- ✅ 已完成：FSA 本地优先架构、卡片瘦身 + 一行流索引 + agent 渐进式披露协议、荧光手帐 UI、Agent 提示（路径可填一次自动补全）。`npm test` 26/26。
